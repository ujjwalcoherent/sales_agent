...
import ProfileFormPanel from "./_components/ProfileFormPanel";
...
> = {
  ...
  company_first: { label: "Company-First", bg: "var(--blue-light)", color: "var(--blue)" },
...
export default function ProfilesPage() {
  ...
  // Form panel
  const [panelProfile, setPanelProfile] = useState<UserProfile | null | undefined>(undefined);
  // undefined = closed, null = new, UserProfile = edit
  ...
  function openEdit(e: React.MouseEvent, profile: UserProfile) {
    e.stopPropagation();
  ...
  function handleSaved(saved: UserProfile) {
    setProfiles((prev) => {
      const existing = prev.findIndex((p) => p.profile_id === saved.profile_id);
      if (existing >= 0) {
        const next = [...prev];
        next[existing] = saved;
        return next;
  ...
  function requestDelete(e: React.MouseEvent, profile: UserProfile) {
    e.stopPropagation();
    setDeleteTarget({ id: profile.profile_id, name: profile.user_name });
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setProfiles((prev) => prev.filter((p) => p.profile_id !== deleteTarget.id));
    } catch (err) {
    ...
    <>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button
                onClick={() => setDeleteTarget(null)}
                style={{ padding: "8px 16px", fontSize: 13, borderRadius: 8, border: "1px solid var(--border)", background: "var(--surface-raised)", color: "var(--text-secondary)", cursor: "pointer" }}
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                style={{ padding: "8px 16px", fontSize: 13, fontWeight: 600, borderRadius: 8, border: "none", background: "var(--red)", color: "#fff", cursor: "pointer" }}
              >
                Delete
              </button>
            </div>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "18px 24px" }}>
        ...
        {loading ? (
          ...
          >
            <User size={36} style={{ color: "var(--border-strong)", marginBottom: 14 }} />
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text-secondary)", marginBottom: 6 }}>
              No profiles yet
            </div>
            <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.7, maxWidth: 380, marginBottom: 20 }}>
          ...
          >
            {profiles.map((profile) => {
              ...
              const visibleIndustries = profile.target_industries.slice(0, 4);
              const extraIndustries = profile.target_industries.length - 4;
              ...
              return (
                ...
                >
                  ...
                  {/* Avatar + name */}
                  <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12, paddingRight: 80 }}>
                    <div
                      style={{
                        width: 40,
                        height: 40,
                  ...
                  >
                    ...
                    <span style={{ color: "var(--text-muted)" }}>{profile.region}</span>
                  </div>
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => requestDelete(e, profile)}
