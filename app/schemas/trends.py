...
class TrendNode(BaseModel):
    ...
    popularity_score: float = 0.0            # Raw article count (with decay)
    ...
    # ── Evidence fields (E1: source article details for transparency) ──
    source_article_titles: List[str] = Field(default_factory=list)
    source_article_urls: List[str] = Field(default_factory=list)
    source_article_sources: List[str] = Field(default_factory=list)
    ...
    # ── Cross-trend intelligence (Feedly Leo entity linking) ──
    related_trend_ids: List[str] = Field(default_factory=list)
    related_trend_titles: List[str] = Field(default_factory=list)
