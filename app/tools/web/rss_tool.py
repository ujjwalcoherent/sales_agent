...
from app.config import get_settings, NEWS_SOURCES, DEFAULT_ACTIVE_SOURCES
...
class RSSTool:
    ...
    async def fetch_all_sources(
        ...
        concurrency = self.settings.rss_fetch_concurrency
        per_source_timeout = self.settings.rss_per_source_timeout
        ...
        async def _fetch_limited(sid):
            async with semaphore:
                try:
                    ...
                    logger.warning(f"[TIMEOUT] {NEWS_SOURCES.get(sid, {}).get('name', sid)}: Fetch timeout ({per_source_timeout}s), skipping")
                    return []
    ...
    async def _fetch_rss_source(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        async with httpx.AsyncClient(timeout=self.settings.rss_httpx_timeout) as client:
    ...
    _API_DISPATCH = {
        ...
        "gdelt_global_tech": "_fetch_gdelt",
        "gdelt_india_funding": "_fetch_gdelt",
        "gdelt_india_regulation": "_fetch_gdelt",
    }
    ...
    async def _fetch_newsapi_org(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        _country = self.settings.country
        params = {
            "apiKey": api_key,
            "q": f"{_country} business OR {_country} economy OR {_country} startup",
            "language": "en",
            "sortBy": "publishedAt",
    ...
    async def _fetch_rapidapi_news(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        params = {
            "query": f"{_settings.country} business",
            "country": _settings.country_code,
            "lang": "en",
    ...
    async def _fetch_google_trends_news(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        _cc = self.settings.country_code.lower()
        _country = self.settings.country
        requests = [
            (f"{base}/news", {"q": f"{_country} business economy", "country": _cc, "language": "en", "limit": str(per_endpoint)}),
            (f"{base}/top-headlines", {"country": _cc, "language": "en", "limit": str(per_endpoint)}),
            (f"{base}/topic-headlines", {"country": _cc, "language": "en", "topic": "Business", "limit": str(per_endpoint)}),
        ]
    ...
    async def _fetch_mediastack(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        params = {
            "access_key": api_key,
            "countries": self.settings.country_code.lower(),
            "languages": "en",
            "categories": "business",
    ...
    async def _fetch_thenewsapi(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        params = {
            "api_token": api_key,
            "locale": self.settings.country_code.lower(),
            "language": "en",
            "categories": "business",
    ...
    async def _fetch_webz(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        params = {
            "token": api_key,
            "q": f"site_category:business language:english site_country:{self.settings.country_code}",
    ...
    async def _fetch_gnews(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        params = {
            "token": api_key,
            "country": self.settings.country_code.lower(),
            "category": "business",
    ...
    async def _fetch_gdelt(self, source: Dict, max_results: int) -> List[NewsArticle]:
        ...
        _GDELT_QUERIES: dict[str, str] = {
            # India broad — unchanged
            "gdelt_india": "sourcecountry:IN sourcelang:english",
            # India business — keyword-gated
            "gdelt_india_business": (
                "sourcecountry:IN sourcelang:english "
                "(business OR startup OR economy OR funding OR acquisition OR regulation)"
            ),
            # India VC/IPO/M&A — deal-flow specific
            "gdelt_india_funding": (
                "sourcecountry:IN sourcelang:english "
                "(\"Series A\" OR \"Series B\" OR \"Series C\" OR funding OR IPO OR \"venture capital\" "
                "OR acquisition OR merger OR unicorn)"
            ),
            # India regulatory — RBI, SEBI, government policy
            "gdelt_india_regulation": (
                "sourcecountry:IN sourcelang:english "
                "(RBI OR SEBI OR \"ministry of finance\" OR regulation OR compliance OR policy "
                "OR \"data protection\" OR \"DPDP\" OR GST)"
            ),
            # Global B2B tech — not country-restricted, keyword-gated
            "gdelt_global_tech": (
                "sourcelang:english "
                "(\"enterprise software\" OR SaaS OR \"artificial intelligence\" OR cybersecurity "
                "OR \"cloud computing\" OR fintech) "
                "(funding OR acquisition OR \"product launch\" OR partnership)"
            ),
        }
        query = _GDELT_QUERIES.get(source_id, "sourcecountry:IN sourcelang:english")
        ...
        params = {
            ...
            "sort": "DateDesc",
            # Use configured lookback window (hours_ago from settings, default 120h = 7200min)
            "timespan": str(getattr(self.settings, 'rss_hours_ago', 120) * 60),
        }
