...
from app.tools import json_repair
...
class LLMService:
    ...
    _logged_modes: set = set()  # Track which mode messages have been logged
    ...
    def __init__(self, mock_mode: bool = False, force_groq: bool = False, lite: bool = False, disabled_providers: list[str] | None = None):
        ...
        mode_key = f"{'lite' if lite else 'full'}_{self.mock_mode}"
        if mode_key not in LLMService._logged_modes:
            LLMService._logged_modes.add(mode_key)
    ...
    async def run_structured(
        ...
        reflect_retries: int = 2,
    ) -> T:
        """Generate structured output validated by pydantic-ai.

        Uses ReflectAndRetry pattern: when structured output fails validation,
        inject the error message back into the prompt and retry on the SAME
        provider before triggering provider failover. Most parse failures are
        minor formatting issues that self-correct with error feedback.

        REF: Google ADK ReflectAndRetry plugin, Anthropic error-feedback loop
        """
        agent = await self._get_agent_with_cooldown_wait(output_type, system_prompt, retries)
        last_error = None

        for attempt in range(1 + reflect_retries):
            effective_prompt = prompt
            if last_error:
                effective_prompt = (
                    f"{prompt}\n\n"
                    f"[PREVIOUS ATTEMPT FAILED WITH ERROR: {last_error}\n"
                    f"Fix your output to match the required schema exactly. "
                    f"Return ONLY valid JSON matching the schema.]"
                )
                logger.info(
                    f"ReflectAndRetry: attempt {attempt + 1}/{1 + reflect_retries} — "
                    f"injecting error feedback to self-correct"
                )
            ...
            try:
                ...
                # Check if this is a validation/parse error (retryable) vs provider error (not)
                exceptions = getattr(eg, 'exceptions', [eg])
                is_validation_error = any(
                    "validation" in str(e).lower() or
                    "json" in str(e).lower() or
                    "parse" in str(e).lower() or
                    "field required" in str(e).lower()
                    for e in exceptions
                )

                if is_validation_error and attempt < reflect_retries:
                    # Self-correct: inject the error and retry on same provider
                    last_error = "; ".join(str(e)[:200] for e in exceptions[:2])
                    continue
                else:
                    # Provider-level failure — record cooldowns and propagate
                    self._process_failures(eg)
                    raise RuntimeError(f"All LLM providers failed: {eg}") from eg
            except Exception as e:
                # Catch pydantic validation errors directly (not wrapped in ExceptionGroup)
                err_str = str(e).lower()
                is_validation = (
                    "validation" in err_str or "json" in err_str or
                    "parse" in err_str or "field required" in err_str
                )
                if is_validation and attempt < reflect_retries:
                    last_error = str(e)[:300]
                    continue
                raise

        # Should not reach here, but just in case
        raise RuntimeError(f"ReflectAndRetry exhausted after {1 + reflect_retries} attempts")
