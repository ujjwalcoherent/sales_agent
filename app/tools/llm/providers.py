...
class ProviderManager:
    ...
    _chain_logged: bool = False  # Log the chain only once per session
    ...
    def _get_available_providers(self) -> List[Tuple[str, "Model"]]:
        ...
        self._provider_order = [name for name, _ in providers]
        if providers and not ProviderManager._chain_logged:
            ProviderManager._chain_logged = True
            logger.info(f"LLM chain: {' → '.join(self._provider_order)} (primary: {self._provider_order[0]})")
        return providers
        ...
        if self.settings.gcp_project_id and self.settings.gcp_service_account_file:
            ...
            if os.path.exists(sa_path):
                try:
                    ...
                    logger.debug(f"Built Vertex AI fallback (project={self.settings.gcp_project_id}, "
